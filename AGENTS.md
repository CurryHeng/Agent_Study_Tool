...
> 当前迭代：冲刺（截止 2026-08-20），四人分工见 `docs/（必读）任务分工.md`（P0-P3 按编码顺序，GitHub issues #33-#63 一一对应）。
...
## 13. 项目进度与当前状态（P0 已完成，冲刺进行中）

> 2026-08-15：项目正式更名 **EStudy**（原 StudyForge）；P0 缺口修补完成（上传自动索引/auth 同步/文档对齐/Agent 真实链路 bug 修复/FSRS-6 升级）；架构定稿为 1 主 Agent + 3 专家（§5）。
> 冲刺任务分级（详见 `docs/（必读）任务分工.md`）：
> - **P0 演示阻塞**：聊天页（A）、出题 UI（B）、导图修复（C）、文件导入真实验证（C）、**主 Agent ReAct 化（D）**
> - **P1 主任务**：契约冻结（D 主持）、知识编辑接口（C）、错因分析（B）、两阶段确认（D）、上下文+多轮（D）、出题调优（B）、填空判分（B）、学习热力图（A）、对话历史（D 后端 + A 前端）、联调（A）
> - **P2 增强 / P3 质量债**：见任务分工文档与 issues。
