...
import { agentApi, workbookApi } from '../api'
import type {
  AgentChatContext,
  AgentProposal,
  AgentStep,
...
// #41 上下文注入：优先把“当前页面/选中节点”告诉 Agent，便于“这里/这个”指代消解
const context = computed<AgentChatContext>(() => ({
  view: String(route.name ?? route.path ?? 'unknown'),
...
async function send(text?: string) {
  ...
  try {
    ...
    if (resp.conversation_id != null) conversationId.value = resp.conversation_id
...
<template>
  <div class="mx-auto flex h-[calc(100vh-7rem)] max-w-4xl flex-col animate-fade-in">
    <!-- 头部：标题 + 练习册选择 -->
    ...
    >
      <!-- 空状态 -->
      <div v-if="messages.length === 0 && !loading" class="flex h-full flex-col items-center justify-center py-12 text-center">
        <div
        ...
        <div v-else class="flex justify-start">
          <div class="w-full max-w-[92%] space-y-2">
            <div class="flex items-start gap-2.5">
              ...
              <div class="min-w-0 flex-1 rounded-2xl rounded-tl-sm border border-slate-200/80 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
            ...
            >
              ...
              <ul v-if="msg.stepsOpen" class="space-y-1.5 border-t border-slate-200/70 px-3 py-2 dark:border-slate-700">
                <li
                  v-for="(step, si) in msg.steps"
                  :key="si"
                  class="flex items-start gap-2 text-xs"
                >
                  <CircleCheck v-if="step.ok" :size="14" class="mt-0.5 shrink-0 text-emerald-500" />
            ...
            <div v-if="msg.proposals && msg.proposals.length" class="ml-9 space-y-2">
              ...
              >
                <div class="flex items-start justify-between gap-2">
                  <div class="min-w-0">
                    <p class="text-sm font-semibold text-slate-800 dark:text-white">
                      ...
                      <span v-if="targetSummary(state.proposal)" class="font-normal text-slate-500 dark:text-slate-400">
                        · {{ targetSummary(state.proposal) }}
                      </span>
                    </p>
                    <p v-if="state.proposal.impact" class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                ...
                <div
                  v-if="changesSummary(state.proposal).before != null || changesSummary(state.proposal).after != null"
